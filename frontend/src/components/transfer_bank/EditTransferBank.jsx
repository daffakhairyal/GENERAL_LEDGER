...
const EditTransferBank = ({ isVisible, onClose, transferBankEntryId }) => {
    ...
    useEffect(() => {
        const fetchEntryData = async () => {
            try {
                ...
                console.error('Error fetching transfer bank entry data:', error);
            }
        };
    ...
    const handleSave = async () => {
        try {
            await axios.patch(`http://localhost:5000/transfer_bank/${transferBankEntryId}`, entryData);
            onClose();
        } catch (error) {
            console.error('Error updating transfer bank entry:', error);
        }
    };
    ...
    return (
        <div className={`fixed inset-0 flex justify-center items-center z-50 overflow-auto bg-black bg-opacity-50 transition-opacity duration-300 ${isVisible ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
            <div className="bg-white p-8 rounded-lg shadow-md w-[600px] animate-fade-in">
                <h2 className="text-xl font-semibold mb-4">Edit Transfer Bank</h2>
...
export default EditTransferBank;
